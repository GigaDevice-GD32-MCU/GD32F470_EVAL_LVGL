 ...
 *      DEFINES
 *********************/
#define SCALE_POS_X(position) ((lv_coord_t)((position) * LV_HOR_RES_MAX / 1024))
#define SCALE_POS_Y(position) ((lv_coord_t)((position) * LV_VER_RES_MAX / 600))
...
static void widget_event_cb(lv_event_t *e)
{
    ...
    /* 在 lv_demo_widgets() 内部会 lv_obj_clean(lv_scr_act())，
     * 必须先停掉仍引用主页对象的定时器，否则可能访问已释放内存导致卡死。*/
    app_stop_page_timers();
    lv_demo_widgets();
}
...
static void update_demo_performance_stats(void)
{
    ...
    static uint32_t random_state = 0x470F470U;
    ...
    g_fps = 40U - g_cpu / 8U + ((random_state >> 24) & 0x01U);
    if(g_fps < 28U) g_fps = 28U;
...
//    lv_obj_set_size(imgbtn_file, 100, 100);
    ...
    lv_obj_align_to(label_widget, imgbtn_widget, LV_ALIGN_OUT_BOTTOM_MID, -48, -90);
    ...
    lv_obj_align_to(label_music, imgbtn_music, LV_ALIGN_OUT_BOTTOM_MID,  -48, -90);
    ...
    lv_obj_align_to(label_speed, imgbtn_speed, LV_ALIGN_OUT_BOTTOM_MID, -48, -90);
    ...
    lv_obj_align_to(label_picture, imgbtn_picture, LV_ALIGN_OUT_BOTTOM_MID, -48, -90);
    ...
    lv_obj_align_to(label_about, imgbtn_about, LV_ALIGN_OUT_BOTTOM_MID, -48, -90);
    ...
    lv_obj_align_to(label_qrcode, imgbtn_qrcode, LV_ALIGN_OUT_BOTTOM_MID, -48, -90);
    ...
    lv_obj_align_to(label_file, imgbtn_file, LV_ALIGN_OUT_BOTTOM_MID, -48, -90);
    ...
    lv_obj_align_to(label_setting, imgbtn_setting, LV_ALIGN_OUT_BOTTOM_MID, -48, -90);
    ...
    /* 用符合实际曲线走势的虚拟数据预先填满两个图表，避免刚进入性能页时是空的 */
...
//    lv_obj_set_style_text_font(font_label, &lv_font_digital_35, LV_STATE_DEFAULT);
    ...
    if(touch_cursor == NULL) {
        ...
        lv_obj_set_size(touch_cursor, SCALE_POS_X(20), SCALE_POS_Y(20));
